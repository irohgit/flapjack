...
func _ready() -> void:
	print("stage_2 ready | in tree: ", is_inside_tree(), " | parent: ", get_parent())
	Audio.play_music(stage_music, stage_music_db, music_fade_in)
	assert(scroll_director != null, "Stage 2 needs a ScrollDirector")
	assert(enemy_controller != null, "Stage 2 needs an EnemyController")
	assert(level_generator != null, "Stage 2 needs a LevelGenerator")
...
func run_special_encounter(encounter: GeneratedEncounter) -> void:
	print("is inside tree: ", is_inside_tree(), " | parent: ", get_parent())
